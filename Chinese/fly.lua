...
local toggleBtn
toggleBtn = addToggle("开关 速度: " .. speeds, function(enabled)
	local char = speaker.Character
...
local incraseBtn
incraseBtn = addButton("加速", function()
	local char = speaker.Character
...
local decraseBtn
decraseBtn = addButton("减速", function()
	local char = speaker.Character
	if char and char.Humanoid then
		local hum = char.Humanoid
