...
function InitMain()
	...
	Title.Text = "项目名称：胡说八道\n制作者: 种族主义者 Dolphin#5199\n版本 3.5.5 (正在进行的返工)"
	...
	TextLabel.Text = [[
脚本 由: 种族主义者 Dolphin#5199
GUI 作者：一个想隐藏他的名字的人

隐藏/显示 GUI，按 "P" 键

注意 #1：由于我的字符串操作技能不是最好的，因此更改 esp/cham 颜色可能会很麻烦。
注意 #2：黑名单功能将回归！我只是没有足够的时间来制作 gui
注意 #3：保存设置可能仍然有问题。如果它仍然搞砸了，请给我发消息

这适用于每款游戏，但 Aimbot 则不起作用！（不适用于：越狱和幻影部队）

常见问题：
1） 我该如何使用瞄准机器人？
A： 激活它，并在游戏中按住右键。瞄准机器人将锁定最近的敌人，而不是在墙后。（如果该玩家在墙后面，它将找到下一个不在墙后面最近的玩家。

2） ESP/Chams 在我玩的游戏上不起作用？
答：有些游戏需要我制作补丁（例如：Murder Mystery、Murder Mystery X）以请求补丁或游戏在 Discord 上给我发消息。

3） 我如何检测玩家何时在墙后？
A： 将摄像机光线投射到其他玩家。

4） 使用 aimbot 时我的子弹仍然没有命中？！
答：责怪子弹散布，尽量控制你开火的频率。（谋杀之谜 2 = 垃圾）（为什么单发手枪他妈的有子弹散布？哈哈，什么鬼？
...
3/10/2018:
+ 修复了玩家重生时 chams 如何损坏的问题。

3/10/2018:
+ 修复了 ESP 无法正确更新的问题。
+ 修复了 Chams 无法正确更新的问题。（也许吧？我不知道是什么打破了这个）

3/9/2018:
+ 生物 ESP/Chams！（测试版！

3/8/2018:
+ 修复了未为 esp/chams/tracer 长度输入有效数字时出现的错误。
+ 修复了 aimlock 的滞后问题。
+ 修复了 chams 的滞后问题。

3/8/2018:
+ 谋杀补丁 15
- 暂时删除了自动触发功能，因为 mouse1click 在 Synapse :( 上断开
...
3/6/2018:
- 移除了 ：IsFriendsWith 检查。（请改用好友列表 GUI）
...
3/4/2018:
+ 固定十字准线切换。
+ Island Royal 的 Aimlock 补丁。
+ 最后修复保存设置。

3/4/2018:
+ 为 1968 部队：越南修复的 Aimlock
+ 瞄准锁的自动开火设置
+ 修复了有时必须双击按钮才能激活选项的问题

3/4/2018:
+ 修复了 FreeForAll 设置错误。
+ 在 Phantom Forces / Jailbreak 上使用 aimlock 现在会告诉您它不会工作。
* 将 Aimbot 重命名为 Aimlock

3/3/2018:
+ 重新添加了黑名单功能。
+ Aimbot 将不再关注黑名单中的人。
+ 兼容具有 readfile 和 writefile 的漏洞。

3/3/2018:
+ GUI 大修
+ Aimbot 现在只瞄准不在墙后的人
+ 当屏幕上显示 x player 时，Chams 现在会变暗。
+ Chams 不再具有人形根部。（欢迎）
+ Silent Assassin 补丁
+ 我的 discord 被删除了，所以我现在正在使用 pastebin。（自动更新:)
]]
	...
	MobESPButton.MouseButton1Click:connect(function()
		...
		if Bullshit.MobESP then
			local hint = Instance.new("Hint", CoreGui)
			hint.Text = "关闭 ESP/Chams 并再次打开以查看 mob ESP。"
			wait(5)
			hint.Text = "这仍处于测试阶段，预计会出现问题！如果您遇到错误，请在 discord 上向 Racist Dolphin#5199 发送消息！"
			wait(10)
			hint:Destroy()
		end
	end)
	...
	MobChamsButton.MouseButton1Click:connect(function()
		...
		if Bullshit.MobChams then
			local hint = Instance.new("Hint", CoreGui)
			hint.Text = "关闭 ESP/Chams 并再次打开以查看 mob chams。"
			wait(5)
			hint.Text = "这仍处于测试阶段，预计会出现问题！如果您遇到错误，请在 discord 上向 Racist Dolphin#5199 发送消息！"
			wait(10)
			hint:Destroy()
...
Run:BindToRenderStep("UpdateInfo", 1000, function()
	...
	if Bullshit.DebugInfo then
		...
		if Bullshit.ClosestEnemy ~= nil then
			...
			DebugMenu["PlayerSelected"].Text = "最近的敌人: " .. tostring(Bullshit.ClosestEnemy)

			if Bullshit.ClosestEnemy.Team ~= nil then
				DebugMenu["PlayerTeam"].Text = "团队: " .. tostring(Bullshit.ClosestEnemy.Team)
			else
				DebugMenu["PlayerTeam"].Text = "团队: nil"
			end

			if GetHum then
				DebugMenu["PlayerHealth"].Text = "血量: " .. string.format("%.0f", GetHum.Health)
			end
			if MyTor and GetTor then
				local Pos = GetTor.Position
				local Dist = (MyTor.Position - Pos).magnitude
				DebugMenu["PlayerPosition"].Text = "位置: (X: " .. string.format("%.3f", Pos.X) .. " Y: " .. string.format("%.3f", Pos.Y) .. " Z: " .. string.format("%.3f", Pos.Z) .. ") 距离: " .. string.format("%.0f", Dist) .. " 螺栓"
				...
				if part == GetTor then
					DebugMenu["BehindWall"].Text = "墙后: false"
				else
					DebugMenu["BehindWall"].Text = "墙后: true"
				end
		...
		if MyTor then
			...
			DebugMenu["Position"].Text = "我的位置: (X: " .. string.format("%.3f", Pos.x) .. " Y: " .. string.format("%.3f", Pos.Y) .. " Z: " .. string.format("%.3f", Pos.Z) .. ")"
		end
		...
		DebugMenu["FPS"].Text = "FPS: " .. tostring(fps) .. " 平均: " .. string.format("%.0f", (sum / #Bullshit.FPSAverage))
